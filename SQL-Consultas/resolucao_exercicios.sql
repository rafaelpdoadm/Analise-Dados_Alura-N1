...
SELECT nome || ' ' || sobrenome AS nome_completo, length(nome || ' ' || sobrenome) AS compimento_nome
FROM clientes;
...
SELECT id_vendas, datavenda, JULIANDAY ('now') - JULIANDAY (datavenda) AS diferenca_dias
FROM vendas;
